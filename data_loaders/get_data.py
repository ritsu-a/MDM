...
def get_dataset(name, num_frames, split='train', hml_mode='train', abs_path='.', fixed_len=0,
                device=None, autoregressive=False, cache_path=None, data_dir: str = "", norm_data_dir: str = ""):
    DATA = get_dataset_class(name)
    if name == "motion_stat_300":
        dataset = DATA(split=split, num_frames=num_frames, mode=hml_mode, abs_path=abs_path, fixed_len=fixed_len,
                       device=device, autoregressive=autoregressive, cache_path=cache_path, data_dir=data_dir, norm_data_dir=norm_data_dir)
    elif name == "beat_v2":
        dataset = DATA(split=split, num_frames=num_frames, mode=hml_mode, abs_path=abs_path, fixed_len=fixed_len,
